...
# 1. Remove global_drag usage
content = re.sub(r'\.on_drag\(global_drag\([^)]+\)\)\n\s*\.grow\(\)', '.grow()', content)

# 2. Add pager_drag and summary_drag functions
drag_code = """
fn pager_drag(
    strip_tx: Signal<f64>,
...
# 3. Add on_drag to week_view inner row (WITHOUT grow())
...
# 4. Add on_drag to summary_view inner row (WITHOUT grow())
content = content.replace("""                    .translation(strip_tx, 0.0),
                ))
            },
        ),
...
# 5. Remove unused variables in render
content = re.sub(r'let drag_tx = strip_tx\.clone\(\);\n', '', content)
content = re.sub(r'let drag_width = page_width\.clone\(\);\n', '', content)
content = re.sub(r'let drag_state = state\.clone\(\);\n', '', content)
content = re.sub(r'let show_summary_drag = show_summary\.clone\(\);\n', '', content)
content = re.sub(r'fn global_drag.*?^}\n', '', content, flags=re.MULTILINE|re.DOTALL)
