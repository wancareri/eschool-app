...
for i, line in enumerate(lines):
    if "pull_to_refresh(refreshing, scroll(column((" in line:
        out.append("    zstack((")
        out.append("        rect().color(Color::rgba(255.0, 255.0, 255.0, 0.02)).grow(),")
        out.append(line)
    elif "features::diary::load_all(state);" in line:
        out.append(line)
        out.append("        }")
        out.append("    })")
        out.append("    )) // close zstack")
        out.append("    .on_drag(global_drag(strip_tx.clone(), page_width.clone(), state.clone(), show_summary.clone()))")
        out.append("    .grow()")
        out.append("}")
        break
    else:
